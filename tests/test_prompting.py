...
class TestPromptConfigHash:
    ...
    def test_distinct_styles_have_distinct_hashes(self, acs_income_task):
        ...
        assert hash(base) != hash(PromptConfig.from_dict({"connector": "="}, task=acs_income_task))
        assert hash(base) != hash(PromptConfig.from_dict({"format": "comma"}, task=acs_income_task))
...
class TestEncodeRowPrompt:
    ...
    def test_default_connector_matches_main(self, acs_income_task, acs_row):
        ...
        colon_less = encode_row_prompt(
            ...
            prompt_config=PromptConfig.from_dict({"connector": "is"}, task=acs_income_task),
        )
...
class TestEncodeRowPromptFewShot:
    ...
    def test_examples_repeat_question_by_default(self, acs_income_task, acs_income_dataset, acs_row):
    ...
    def test_hide_question_in_examples_shows_question_once(self, acs_income_task, acs_income_dataset, acs_row):
        ...
        prompt = encode_row_prompt_few_shot(
            ...
            task=acs_income_task,
            dataset=acs_income_dataset,
            few_shot_config=FewShotConfig(n_shots=2, reuse_examples=True, show_question_in_examples=False),
        )
        question_text = acs_income_task.question.get_question_prompt()
        assert prompt.count(question_text) == 1
        assert prompt.endswith(question_text)

    def test_few_shot_task_description_matches_main_wording(self, acs_income_task, acs_income_dataset, acs_row):
