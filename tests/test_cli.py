...
from folktexts.cli.run_benchmark import setup_arg_parser
...
class TestGetOrCreateResultsDir:
    ...
    def test_slashes_in_model_name_replaced(self, tmp_path):
        result = get_or_create_results_dir("org/gpt2", "ACSIncome", tmp_path)
        assert "/" not in result.name
        assert "org--gpt2" in result.as_posix()
    ...
    def test_no_double_append_if_already_correct(self, tmp_path):
        r1 = get_or_create_results_dir("gpt2", "ACSIncome", tmp_path)
        r2 = get_or_create_results_dir("gpt2", "ACSIncome", r1)
        assert r1 == r2
...
class TestSetupArgParser:
    ...
    def _required(self):
        return ["--model", "gpt2", "--results-dir", "/tmp/results", "--data-dir", "/tmp/data"]
    ...
    def test_boolean_flag_use_generated_text(self):
        args = self._parse(self._required() + ["--use-generated-text"])
        assert args.use_generated_text is True

    def test_reasoning_string_value(self):
        args = self._parse(self._required() + ["--reasoning", "low"])
        assert args.reasoning == "low"
        assert isinstance(args.reasoning, str)

    def test_reasoning_int_value(self):
        args = self._parse(self._required() + ["--reasoning", "1024"])
        assert args.reasoning == 1024
        assert isinstance(args.reasoning, int)

    def test_reasoning_float_value(self):
        args = self._parse(self._required() + ["--reasoning", "0.5"])
        assert args.reasoning == pytest.approx(0.5)
        assert isinstance(args.reasoning, float)
    ...
    def test_variation_parsed_as_dict(self):
        args = self._parse(self._required() + ["--variation", "format=bullet;connector=is"])
