...
class BenchmarkConfig:
    ...
    enable_thinking : bool, optional
        ...
        Qwen3). Only applies when text generation is used. When enabled, calls
...
class Benchmark:
    ...
    def configs_dict(self) -> dict:
        ...
        # null for some question types (if None is default), which load_from_disk cannot
        # distinguish from an explicit None (disable-role), breaking benchmark reproduction.
        for key in ("system_prompt", "chat_prompt"):
    ...
    def run(
        ...
        if count_nan > 0:
            ...
            logging.info(
                ...
                + (f"s_test: {s_test.to_numpy().shape} -> {s_test.to_numpy()[nan_mask].shape}" if s_test is not None else "")
            )
    ...
    def make_tableshift_benchmark(
        ...
        **kwargs
        ...
        tableshift_task = TableshiftBRFSSTaskMetadata.get_task(name=task_name, use_numeric_qa=config.numeric_risk_prompting)
    ...
    def _validate_config(config: BenchmarkConfig) -> None:
        ...
        if config.use_chat_template and config.enable_thinking:  # (config.cot_prompting or config.enable_thinking)
            raise ValueError(
        ...
        # TODO: Check if still valid to raise error here
